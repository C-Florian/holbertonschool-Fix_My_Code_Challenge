...
class User:
    ...
    def __init__(self):
        """Initialize a new user with a unique id and no password set."""
        self.id = str(uuid.uuid4())
        self.__password = None  # private instance attribute

    @property
    def password(self):
        """Getter: returns the stored MD5 hash (never the raw password)."""
    ...
    def password(self, pwd):
        """
        Setter rules:
        - set to None if pwd is None or not a string
        - otherwise store MD5(pwd) as a lowercase hex digest
        """
        if pwd is None or not isinstance(pwd, str):
            self.__password = None
        else:
            self.__password = hashlib.md5(pwd.encode("utf-8")).hexdigest()

    def is_valid_password(self, pwd):
        """
        Returns True iff:
        - pwd is a string,
        - a password hash is set,
        - MD5(pwd) matches the stored hash.
        """
        if pwd is None or not isinstance(pwd, str):
            return False
        if self.__password is None:
            return False
        return hashlib.md5(pwd.encode("utf-8")).hexdigest() == self.__password
